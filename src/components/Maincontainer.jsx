...
const Maincontainer = ({extended, setExtended, isDarkMode}) => {
    ...
    return (
            <div className={`sm:w-[80%] w-[100%] 2xl:w-[65%] sm:h-[90vh] h-[95vh] grid sm:grid-rows-[75%_25%] grid-rows-[90%_10%] mx-auto `}>
                ...
                <div className={`${isDarkMode && `bg-[#131314]`} transition-all duration-200`}>
                    {showResult ? <GetResult question={question} resultData={resultData} loading={loading} isDarkMode={isDarkMode} /> : <Greetings isDarkMode={isDarkMode} /> }
                </div>
                <div id="input-section" className={`flex items-end pb-3 ${isDarkMode && `bg-[#131314]`}`}>
...
function Greetings({isDarkMode}){
    return(
        <>
            <div>
                <div id="greet" className='w-[100%] py-10 sm:px-2 px-5 '>
                ...
                <div id="cards" className="w-[100%] px-2 flex  mt-10 ">
                    <div className='sm:w-[100%] h-[100%] sm:flex space-x-4 hidden'>
                        <div className={`w-[200px] h-[200px] ${isDarkMode ? `bg-[#1f1f1f] text-slate-200` : `bg-slate-200`}  opacity-90 p-3 rounded-md flex flex-col justify-between `}>
